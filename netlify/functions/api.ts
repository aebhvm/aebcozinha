...
const SCHEMA_VERSION = '2026-09-17-stock-orders-idempotency-v1'
...
async function migrate() {
  ...
  await client.batch(
    [
      ...
      `create table if not exists manager_report_attachments (
        id integer generated by default as identity primary key,
        report_id integer not null references manager_reports(id) on delete cascade,
        attachment_type text not null check(attachment_type in ('imagem', 'audio', 'video')),
      ...
      `create table if not exists manager_report_reads (
        report_id integer not null references manager_reports(id) on delete cascade,
        user_id integer not null references users(id) on delete cascade,
        verified_at text not null,
        primary key(report_id, user_id)
      )`,
      'create index if not exists idx_manager_reports_created_at on manager_reports(created_at desc)',
      'create index if not exists idx_manager_report_attachments_report on manager_report_attachments(report_id)',
      'create index if not exists idx_manager_report_reads_user on manager_report_reads(user_id, report_id)',
  ...
  await ensureColumn('stock_orders', 'request_fingerprint', 'alter table stock_orders add column request_fingerprint text')
  await client.execute('create unique index if not exists idx_stock_orders_request_fingerprint on stock_orders(requested_by, requested_date, request_fingerprint)')
...
async function markSchemaCurrent() {
  await getDb().batch(
    [
      ...
      `create table if not exists stock_movements (
        id integer generated by default as identity primary key,
        product_id integer not null references products(id),
        movement_type text not null check(movement_type in ('desperdicio', 'saida')),
  ...
  await ensureColumn('stock_movements', 'active', 'alter table stock_movements add column active integer not null default 1')
  await migrateStockMovementTypes()
}

async function migrateStockMovementTypes() {
  await getDb().batch(
    [
      'alter table stock_movements drop constraint if exists stock_movements_movement_type_check',
      "update stock_movements set movement_type = 'desperdicio' where movement_type = 'entrada'",
      "alter table stock_movements add constraint stock_movements_movement_type_check check(movement_type in ('desperdicio', 'saida'))",
    ],
    'write',
  )
}
...
type ManagerReport = DbRow & { attachments: DbRow[] }

async function getManagerReports(user: AuthUser, startDate?: string, endDate?: string): Promise<ManagerReport[]> {
  const conditions = ['r.active = 1']
  const args: Array<string | number> = [user.id]
  if (startDate) {
    conditions.push('substring(r.created_at, 1, 10) >= ?')
    args.push(startDate)
  }
  if (endDate) {
    conditions.push('substring(r.created_at, 1, 10) <= ?')
  ...
  const reports = await getDb().execute({
    sql: `select r.id, r.title, r.body, r.created_by, u.name as created_by_name,
        r.created_at, rr.verified_at
      from manager_reports r
      join users u on u.id = r.created_by
      left join manager_report_reads rr on rr.report_id = r.id and rr.user_id = ?
      where ${conditions.join(' and ')}
      order by r.id desc
      limit 200`,
    args,
  ...
  const attachments = await getDb().execute({
    sql: `select id, report_id, attachment_type, file_name, mime_type, size_bytes, data_url
      from manager_report_attachments
      where report_id in (${placeholders})
      order by id`,
    args: reportIds,
  })
  const attachmentsByReport = new Map<number, DbRow[]>()
  for (const row of attachments.rows) {
    const reportId = Number(row.report_id)
    const current = attachmentsByReport.get(reportId) ?? []
    current.push({ ...row, id: Number(row.id), report_id: reportId, size_bytes: Number(row.size_bytes) })
    attachmentsByReport.set(reportId, current)
  }
  ...
  return reports.rows.map((row) => ({
    ...
    attachments: attachmentsByReport.get(Number(row.id)) ?? [],
  }))
}
...
const managerReportAttachmentSchema = z.object({
  ...
  const expectedPrefix = attachment.attachment_type === 'imagem'
    ...
    : attachment.attachment_type === 'audio'
      ? 'audio/'
      : 'video/'
  if (!attachment.mime_type.startsWith(expectedPrefix)) {
    context.addIssue({ code: 'custom', path: ['mime_type'], message: 'Tipo de mídia inválido.' })
  }
...
const stockOrderCreateSchema = z.object({
  ...
  if (new Set(productIds).size !== productIds.length) {
    context.addIssue({ code: 'custom', path: ['items'], message: 'Cada produto só pode aparecer uma vez no pedido.' })
  }
})

function stockOrderFingerprint(body: z.infer<typeof stockOrderCreateSchema>) {
  return JSON.stringify({
    date: body.date,
    notes: body.notes?.trim() ?? '',
    items: [...body.items]
      .map((item) => ({ product_id: item.product_id, quantity: item.quantity.trim() }))
  ...
  try {
    ...
    if (path === '/stock-orders') {
      ...
      if (event.httpMethod === 'POST') {
        ...
        const body = parseBody(event, stockOrderCreateSchema)
        const requestFingerprint = stockOrderFingerprint(body)
        const existing = await getDb().execute({
          sql: 'select id from stock_orders where requested_by = ? and requested_date = ? and request_fingerprint = ? limit 1',
          args: [user.id, body.date, requestFingerprint],
        })
        if (existing.rows[0]) {
          return json(409, { error: 'Este pedido já foi enviado. Ele não pode ser enviado novamente.' })
        }
        ...
        const order = await getDb().execute({
          sql: 'insert into stock_orders (requested_by, requested_date, request_fingerprint, notes, created_at, updated_at) values (?, ?, ?, ?, ?, ?) returning id',
          args: [user.id, body.date, requestFingerprint, body.notes?.trim() || null, brazilNowIso(), brazilNowIso()],
        })
    ...
    if (path === '/stock-movements') {
      ...
      if (event.httpMethod === 'POST') {
        ...
        const body = parseBody(
          ...
          z.object({
            product_id: z.number().int().positive(),
            movement_type: z.enum(['desperdicio', 'saida']),
        ...
        const productResult = await getDb().execute({
          sql: 'select id, name, unit from products where id = ? and active = 1',
          args: [body.product_id],
        })
        const product = productResult.rows[0]
        if (!product) return json(404, { error: 'Produto nao encontrado ou inativo.' })

        const result = await getDb().execute({
          sql: `insert into stock_movements (product_id, movement_type, quantity, unit, movement_date, notes, created_by)
            values (?, ?, ?, ?, ?, ?, ?) returning id`,
    ...
    if (stockMovementMatch && event.httpMethod === 'PUT') {
      ...
      const result = await getDb().execute({
        sql: `update stock_movements
          set product_id = ?, movement_type = ?, quantity = ?, unit = ?, movement_date = ?, notes = ?
          where id = ? and active = 1 returning id`,
    ...
    if (stockMovementMatch && event.httpMethod === 'DELETE') {
      await requireUser(event)
      const result = await getDb().execute({
        sql: 'update stock_movements set active = 0 where id = ? and active = 1 returning id',
        args: [Number(stockMovementMatch[1])],
      })
      if (!result.rows[0]) return json(404, { error: 'Movimentacao nao encontrada.' })
      return json(200, { ok: true })
    }
    ...
    if (path === '/manager-reports/pending-count' && event.httpMethod === 'GET') {
      const user = await requireUser(event, 'gestor')
      const result = await getDb().execute({
        sql: `select count(*) as total
          from manager_reports r
          left join manager_report_reads rr on rr.report_id = r.id and rr.user_id = ?
          where r.active = 1 and r.created_by <> ? and rr.report_id is null`,
    ...
    if (path === '/manager-reports') {
      ...
      if (event.httpMethod === 'GET') {
        ...
        const startDate = dateSchema.parse(url.searchParams.get('startDate') || undefined)
        const endDate = dateSchema.parse(url.searchParams.get('endDate') || undefined)
        if (startDate && endDate && startDate > endDate) {
          return json(400, { error: 'A data inicial não pode ser posterior à data final.' })
        }
    ...
    const managerReportVerifyMatch = path.match(/^\/manager-reports\/(\d+)\/verify$/)
    if (managerReportVerifyMatch && event.httpMethod === 'POST') {
      const user = await requireUser(event, 'gestor')
      const reportId = Number(managerReportVerifyMatch[1])
      const exists = await getDb().execute({
        sql: 'select id from manager_reports where id = ? and active = 1',
        args: [reportId],
      })
      if (!exists.rows[0]) return json(404, { error: 'Relatório não encontrado.' })
      const verifiedAt = brazilNowIso()
      await getDb().execute({
        sql: `insert into manager_report_reads (report_id, user_id, verified_at)
          values (?, ?, ?)
          on conflict(report_id, user_id) do update set verified_at = excluded.verified_at`,
        args: [reportId, user.id, verifiedAt],
      })
      return json(200, { verified_at: verifiedAt })
    }
